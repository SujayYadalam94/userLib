...
uint64_t get_physical_frame(uint64_t va)
{
	...
	if (-1 == (pmfd = syscall_no_intercept(SYS_open, PAGEMAP_PATH, O_RDONLY)))
	...
	// ...and read the entry.
	if (-1 == syscall_no_intercept(SYS_read, pmfd, &pagemap_entry, sizeof(pagemap_entry)))
		die("read(" PAGEMAP_PATH ")");
	if (-1 == syscall_no_intercept(SYS_close, pmfd))
		perror("close(" PAGEMAP_PATH ")");

	// Then, check the value of the `PM_PRESENT` bit...
	if (GET_BIT(pagemap_entry, 63))
		// ...and if it is found to be set, return the corresponding PFN
		return GET_PFN(pagemap_entry);
	// ...otherwise announce its absence and return 0.
	//printf("VA[0x%lx] is not mapped; no physical memory allocated.\n", va);
	return 0;
}

uint64_t get_physical_frame_fast(uint64_t *va, uint64_t lblk)
{
